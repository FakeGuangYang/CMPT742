...
if not args.test:
    ...
    for epoch in range(num_epochs):
        ...
        # print(F1score)
        mAP = generate_mAP(pred_confidence_, pred_box_, ann_confidence_[0].numpy(), ann_box_[0].numpy())
        print("mAP: ", mAP)
    ...
    network.load_state_dict(torch.load('network.pth', map_location=torch.device(device)))
    network.eval()
    ...
    for i, data in enumerate(dataloader_test, 0):
        ...
        with open("predictions" + str(i) + ".txt", "w") as f:
            for box, confidence in zip(pred_box_, pred_confidence_):
                f.write(f"Class: {np.argmax(confidence)}, Confidence: {np.max(confidence)}, Box: {box}\n")
